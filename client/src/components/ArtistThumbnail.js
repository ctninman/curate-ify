...
function ArtistThumbnail({artist, setSingleArtistAlbums}) {
  ...
  return (
    <div 
      className='flex-column-center small-margins' 
      style={{border: '1px solid white', width: '17%'}}
    >
      <img style={{width: '90%', border: '1px solid white', borderRadius: '3px'}} src={artist.artist_photo} />
      <h3 className='small-margins'>{artist.artist_name}</h3>
      <button className='small-margins' onClick={fetchAllArtistAlbums}>All Albums</button>
      <button className='small-margins' onClick={fetchCollectionArtistAlbums}>Albums in My Collection</button>
    </div>
  );
}
